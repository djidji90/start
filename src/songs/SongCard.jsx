// src/components/songs/SongCard.jsx - VERSIÓN FINAL CON DESCARGA VISIBLE
...
const designTokens = {
  ...
  borderRadius: { card: 16, button: 999, menu: 12, dialog: 20, chip: 6 }
};
...
const SongCard = ({ song, showIndex = false, onLike, onMoreActions }) => {
  ...
  const getTooltipText = useCallback(() => {
    switch (primaryState) {
      case 'loading': return `${songStatus?.loadingMessage || 'Cargando'} ${songStatus?.loadingProgress || 0}%`;
      case 'playing': 
  ...
  const handleMainButtonClick = useCallback(async (e) => {
    e.stopPropagation();
    if (isDownloading) { handleCancelDownload(e); return; }
    if (downloadError) { handleRetryDownload(e); return; }
    if (songStatus?.isCurrent) { player.toggle?.(); } 
    else { await player.playSongFromCard?.(song); }
  ...
  const handleCancelDownload = useCallback((e) => {
    ...
    setSnackbar({ open: true, message: '⏹️ Descarga cancelada', severity: 'info' });
    handleMenuClose();
  }, [download, songId]);
  ...
  // ============================================ //
  // RENDER MENÚ (opciones avanzadas)
  // ============================================ //
  const renderMenu = () => {
    const menuItems = [
    ...
    if (isMobile) {
      return (
        <Drawer anchor="bottom" open={Boolean(anchorEl)} onClose={handleMenuClose}
          PaperProps={{ sx: { borderTopLeftRadius: designTokens.borderRadius.dialog, borderTopRightRadius: designTokens.borderRadius.dialog, maxWidth: 480, mx: 'auto' } }}>
          <Box sx={{ p: 2 }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
              <Typography variant="h6" sx={{ fontWeight: 600 }}>Opciones</Typography>
              <IconButton onClick={handleMenuClose} size="small"><CloseIcon /></IconButton>
            </Box>
            {menuItems.map((item, index) => item.divider ? <Divider key={`divider-${index}`} sx={{ my: 1.5 }} /> : (
              <Box key={item.label} onClick={item.onClick} role="menuitem"
                sx={{ display: 'flex', justifyContent: 'space-between', p: 2, borderRadius: designTokens.borderRadius.menu, cursor: 'pointer', color: item.color, '&:hover': { bgcolor: alpha(item.color || designTokens.colors.primary, 0.04) } }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                  <ListItemIcon sx={{ minWidth: 40, color: item.color }}>{item.icon}</ListItemIcon>
                  <ListItemText primary={item.label} primaryTypographyProps={{ fontWeight: 500 }} />
                </Box>
                {item.progress && <Typography variant="caption">{item.progress}</Typography>}
              </Box>
            ))}
          </Box>
        </Drawer>
    ...
    return (
      <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleMenuClose}
        PaperProps={{ sx: { borderRadius: designTokens.borderRadius.menu, minWidth: 220, boxShadow: designTokens.shadows.drawer, mt: 1 } }}>
        {menuItems.map((item, index) => item.divider ? <Divider key={`divider-${index}`} sx={{ my: 1 }} /> : (
          <MenuItem key={item.label} onClick={item.onClick} sx={{ py: 1.5, px: 2 }}>
            <ListItemIcon sx={{ color: item.color, minWidth: 40 }}>{item.icon}</ListItemIcon>
            <ListItemText primary={item.label} primaryTypographyProps={{ fontWeight: 500 }} />
            {item.progress && <Typography variant="caption" sx={{ ml: 2 }}>{item.progress}</Typography>}
          </MenuItem>
        ))}
      </Menu>
    );
  ...
  const renderDialogs = () => (
    <>
      <Dialog open={confirmDialogOpen} onClose={() => setConfirmDialogOpen(false)}
        PaperProps={{ sx: { borderRadius: designTokens.borderRadius.dialog, maxWidth: 400, p: 1 } }}>
        <DialogTitle sx={{ pb: 1 }}>
        ...
        <DialogContent><DialogContentText>¿Eliminar "{song?.title}" de tu dispositivo?</DialogContentText></DialogContent>
        <DialogActions sx={{ p: 2, pt: 0 }}>
          <Button onClick={() => setConfirmDialogOpen(false)}>Cancelar</Button>
          <Button onClick={handleRemoveDownload} variant="contained" color="error">Eliminar</Button>
        </DialogActions>
      </Dialog>

      <Dialog open={downloadInfoDialog} onClose={() => setDownloadInfoDialog(false)}
        PaperProps={{ sx: { borderRadius: designTokens.borderRadius.dialog, maxWidth: 400 } }}>
  ...
  return (
    <>
      <Card onClick={handleCardClick} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}
        sx={{ borderRadius: designTokens.borderRadius.card, overflow: "hidden", cursor: "pointer",
          bgcolor: "#FFFFFF", border: `1px solid ${alpha('#000', 0.1)}`,
          boxShadow: isHovered ? designTokens.shadows.hover : designTokens.shadows.card,
          transition: `all 0.3s ease`, position: "relative" }}>
        
        {/* Barra de progreso */}
        {(songStatus?.isLoading || isDownloading) && (
          <Box sx={{ position: 'absolute', top: 0, left: 0, right: 0, height: 3, zIndex: 10, bgcolor: alpha(currentColor, 0.1) }}>
            <Box sx={{ height: '100%', width: `${songStatus?.isLoading ? songStatus?.loadingProgress || 0 : downloadProgress}%`,
              bgcolor: currentColor, transition: 'width 0.3s linear' }} />
          </Box>
        )}

        <Box sx={{ position: "relative" }}>
          <CardMedia component="img" height={isMobile ? 180 : 220} image={imageUrl} alt={song?.title}
            onError={() => setImageError(true)}
            sx={{ objectFit: "cover", opacity: (songStatus?.isLoading || isDownloading) ? 0.7 : 1 }} />

          {/* Botón principal */}
          <Tooltip title={getTooltipText()} arrow>
            <IconButton onClick={handleMainButtonClick}
              sx={{ position: 'absolute', bottom: 16, right: 16, width: 56, height: 56,
                bgcolor: alpha('#FFFFFF', 0.98), color: currentColor, boxShadow: designTokens.shadows.button,
                backdropFilter: 'blur(8px)', border: `1px solid ${alpha(currentColor, 0.2)}`,
                transition: `all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275)`,
                transform: isActive ? 'scale(1)' : 'scale(0.9)', opacity: isActive ? 1 : 0,
                '&:hover': { bgcolor: '#FFFFFF', transform: 'scale(1.08)' } }}>
              {(songStatus?.isLoading || isDownloading) ? (
                <CircularProgress size={24} value={songStatus?.isLoading ? songStatus?.loadingProgress || 0 : downloadProgress}
                  variant="determinate" sx={{ color: currentColor }} />
              ) : (
                primaryState === 'playing' ? <Pause /> : primaryState === 'paused' ? <PlayArrow /> : 
                downloadError ? <Refresh /> : <PlayArrow />
              )}
            </IconButton>
          </Tooltip>
        </Box>

        {/* Contenido con descarga VISIBLE */}
        <CardContent sx={{ p: 2.5 }}>
          <Box sx={{ display: "flex", alignItems: "flex-start", gap: 1.5 }}>
            {showIndex && (
              <Box sx={{ width: 28, height: 28, borderRadius: "50%", bgcolor: alpha(currentColor, 0.1),
                display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                <Typography variant="caption" sx={{ color: currentColor, fontWeight: 600 }}>{showIndex}</Typography>
              </Box>
            )}
            ...
            <Box sx={{ flexGrow: 1, minWidth: 0 }}>
              ...
              {/* Metadatos + Acciones de descarga VISIBLES */}
              <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1, flexWrap: "wrap" }}>
                  {song?.genre && <Chip label={song.genre} size="small" sx={{ height: 24, fontSize: '0.7rem' }} />}
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                  {isDownloaded ? (
                    <Tooltip title={`Descargada - ${fileSize} MB`}>
                      <CheckCircle sx={{ fontSize: 20, color: designTokens.colors.success, cursor: 'pointer' }}
                    ...
                    <Tooltip title="Descargar canción">
                      <IconButton size="small" onClick={handleDownload}>
                        <Download fontSize="small" sx={{ color: designTokens.colors.primary }} />
                      </IconButton>
                    </Tooltip>
                  )}
                  
                  {/* Menú de 3 puntos para OTRAS opciones */}
