...
// ✅ SIN SHA-256 (mejora rendimiento 20-30%)
// ✅ Compatible con descargas del sistema anterior
// ✅ Manejo inteligente de errores 429 con expiración
...
const useDownload = () => {
  ...
  const getOfflineAudioUrl = useCallback(async (songId) => {
    try {
      ...
      // 3️⃣ Verificar tamaño mínimo (suficiente para integridad básica)
  ...
  // ============================================
  // ✅ verifyDownload (verifica por tamaño, sin SHA-256)
  // ============================================
  const verifyDownload = useCallback(async (songId) => {
    try {
      ...
      // Verificación por tamaño mínimo (suficiente para este caso)
      // Eliminado SHA-256 que ralentizaba sin aportar valor real
  ...
  // ============================================
  // EJECUTAR DESCARGA (VERSIÓN OPTIMIZADA - SIN SHA-256)
  // ============================================
  const executeDownload = useCallback(async (songId, songTitle, artistName) => {
    ...
    try {
      ...
      // 🔥 SOLO guardar en IndexedDB (sin SHA-256 para mejorar rendimiento)
      const obfuscatedKey = await saveToIndexedDB(songId, blob, {
        id: songId,
        title: songTitle,
        artist: artistName,
        fileName
        // Eliminado hash SHA-256 - mejora rendimiento 20-30%
      });
  ...
  }, [getAuthToken, requestSignedUrl, saveToIndexedDB, executeWithRetry, processQueue, log]);
