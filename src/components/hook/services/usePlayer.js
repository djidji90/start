// src/audio/hooks/usePlayer.js - VERSIÓN CON REPEAT
 ...
 * @version 2.2.0 - Agregado soporte para REPEAT
 */
export const useAudioPlayer = () => {
  ...
  /**
   * ✅ NUEVO: Alterna el modo de repetición
   */
  const toggleRepeat = useCallback(() => {
    log('Toggle repeat mode');
    if (player.toggleRepeat) {
      player.toggleRepeat();
    } else {
      log('⚠️ toggleRepeat no disponible en PlayerContext');
    }
  }, [player.toggleRepeat, log]);

  /**
   * ✅ NUEVO: Obtiene el modo de repetición actual
   */
  const getRepeatMode = useCallback(() => {
    return player.repeatMode || false;
  }, [player.repeatMode]);
  ...
  return {
    ...
    // ✅ NUEVO: Repeat mode
    repeatMode: player.repeatMode || false,
    toggleRepeat,
    getRepeatMode,
